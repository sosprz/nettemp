...
CREATE TABLE "lcds" (
	...
	`rows` TINYINT NOT NULL DEFAULT 2,
	`cols` TINYINT NOT NULL DEFAULT 16,
	`clock` TEXT DEFAULT 'off',
	`avg` TEXT DEFAULT 'off',
	`loop` TEXT DEFAULT 'off',
	`active` TEXT DEFAULT 'on',
	`grp` TEXT DEFAULT NULL
);
CREATE TABLE "lcd_groups" (id INTEGER PRIMARY KEY,
	`name` TEXT UNIQUE,
	`active` TEXT DEFAULT 'on',
	`charts` TEXT DEFAULT '',
	`grpkey` TEXT UNIQUE DEFAULT (lower(hex(randomblob(4)))) NOT NULL
);
CREATE TABLE "lcd_group_assign" (
	`rom` TEXT NOT NULL,
	`grpkey` TEXT NOT NULL
);
