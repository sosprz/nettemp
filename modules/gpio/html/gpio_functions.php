...
//humid
    ...
    if ($humidon == "humidon")  {
		...
		// insert to sensors
		$db->exec("INSERT OR IGNORE INTO sensors (position, name, rom, type, alarm, tmp, gpio, adj, charts, device, ch_group) VALUES ('1','$randh','$id_rom_newh', 'humid', 'off', 'wait', '$gpio_post', '0', 'on', 'humid', 'sensors')") or die ("cannot insert to DB humi" );
		$db->exec("INSERT OR IGNORE INTO sensors (position, name, rom, type, alarm, tmp, gpio, adj, charts, device, ch_group) VALUES ('1','$randt','$id_rom_newt', 'temp', 'off', 'wait', '$gpio_post', '0', 'on', 'temp', 'sensors')") or die ("cannot insert to DB temp" );
...
//functiononoff
	...
	if ($gpiodel == "gpiodel")  {
		...
		$db->exec("DELETE FROM g_func WHERE gpio='$gpio_post' AND rom='$rom'");// POPRAWIĆ !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
