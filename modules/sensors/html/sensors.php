...
<?php // SQLite3 - sekcja dodawania do bazy && tworzenie baz rrd
	...
	//DB    
	    if ($type != "relay" ) {
		$db->exec("INSERT OR IGNORE INTO sensors (name, rom, type, alarm, tmp, gpio, device, method, ip, adj, charts, sum, map_pos, map_num) VALUES ('$name','$id_rom_new', '$type', 'off', 'wait', '$gpio', '$device', '$method', '$ip', '0', 'on', '0', '{left:0,top:0}', '$map_num')") or die ("cannot insert to DB" );
	    }
	    if ($type == "relay" ) {
		//relays
