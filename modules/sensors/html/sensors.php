...
<?php // SQLite3 - sekcja dodawania do bazy && tworzenie baz rrd
	...
	$ip='';
	    ...
	    if ( $type != "relay" ) {
		$db->exec("INSERT OR IGNORE INTO sensors (name, rom, type, alarm, tmp, gpio, device, method, ip, adj) VALUES ('$name','$id_rom_new', '$type', 'off', 'wait', '$gpio', '$device', '$method', '$ip', '0' )") or die ("cannot insert to DB" );
	    }
	    if ( $type == "relay" ) {
		//relays
