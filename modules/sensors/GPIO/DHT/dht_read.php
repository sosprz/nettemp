...
try {
	...
	if ( $count >= '1'){
		...
		if(!empty($temp)) {
			...
			shell_exec('php-cgi -f  /var/www/nettemp/receiver.php key=1234 rom=gpio_23_humid value=$local_value type=humid');
		...
		if(!empty($humid)){
			...
			echo $date." Rom: ".$local_rom." Value:".$local_val."\n";
			//db($local_rom,$local_val,$local_type,$local_device,$local_current,$local_ip,$local_gpio,$local_i2c,$local_usb,$local_name);
		    exec('php-cgi -f " /var/www/nettemp/receiver.php key=1234 rom='.$local_rom.' type='.$local_type.' value='.$local_val.'"');
		}
