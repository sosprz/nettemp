...
<?php
    ...
    $sth = $db->prepare("SELECT * FROM sensors WHERE position !=0 AND type!='elec' AND status='on' AND (ch_group='' OR ch_group is null) ORDER BY position ASC");
    $sth->execute();
    $result = $sth->fetchAll(); 
    $numsen = count($result);
    if ($numsen >= 1 ){
...
<?php
   ...
   foreach ($result as $a) {
	...
	$mm='';
		...
		if($a['device'] == 'ip'){ $device='<img src="media/ico/remote.png" alt="" title=IP"/>';}
